...
unsigned char KeyNum, MODE, TimeSetSelect, TimeSetFlashFlag;//时间设置闪烁标志位
//按下按键1，改变MODE
void TimeShow(void)
{
	DS1302_ReadTime();
...
//按键2，设置年月日时分秒并存储在DS1302_Time[]数组中
void TimeSet(void)
{
	...
	// TimeSetFlashFlag每隔0.5s取一次反，下方条件也是间隔0.5s闪烁一次
	...
	//LCD_ShowNum(2, 13, TimeSetFlashFlag, 1);	//TEST: 测试时间设置闪烁标志位
}
...
void main(void)
{
	...
	while(1)
	{
		...
		if(KeyNum == 1)	//Feature1: 按键1连按交替改变按键模式
		{
			...
			{
				MODE = 1;
				TimeSetSelect = 0;/*每次从年开始设置*/
			}
			else if(MODE == 1)
			{
				MODE = 0;
				DS1302_SetTime();
			}	//Feature4: 切换回模式0显示时间时，从数组中读取当前数组中的时间
		}
...
void Timer0_Routine(void) interrupt 1	//定时器/计数器初始化函数
{
	...
	{
		cnt = 0;
		TimeSetFlashFlag = !TimeSetFlashFlag;	//0/1
	}
}
